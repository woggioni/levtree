...
levtree_python_init(levtree_levtree_obj *self, PyObject *args, PyObject *kwds)
{
    ...
    char** carg;
    wchar_t** wcarg; /* argument to pass to the C function*/
    unsigned i;
    ...
    strObj = PyTuple_GetItem(self->wordlist, 0);
    if(PyErr_Occurred())
    {
        return -1;
    }
    wcarg = PyUnicode_AsUnicode(strObj);
    if(PyErr_Occurred())
    {
        self->unicode = 0;
    }
    ...
    if(self->unicode)
    {
        wcarg = malloc(sizeof(wchar_t*)*numLines);
        ...
        for (i=0; i<numLines; i++)
        {
            ...
            /* make it a string */
            wcarg[i] = PyUnicode_AsUnicode(strObj);
        }
        self->wtree = (wlevtree*) malloc(sizeof(wlevtree));
        wlevtree_init(self->wtree,wcarg,numLines);
        free(wcarg);
    }
    else
    {
...
levtree_levtree_search(levtree_levtree_obj* self, PyObject *args, PyObject *kwds)
{
    ...
    if(self->unicode && !PyArg_ParseTupleAndKeywords(args, kwds, "U|iO", kwlist,
                                                      &uwordkey, &number_of_matches,&boolean))
    {
        return NULL;
    }
    else if ( !self->unicode && !PyArg_ParseTupleAndKeywords(args, kwds, "s|iO", kwlist,
    ...
    if(self->unicode)
    {
        wlevtree_search(self->wtree, uwordkey, number_of_matches);
        list = PyList_New(number_of_matches);
        for(i=0; i<number_of_matches; i++)
        {
            res = wlevtree_get_result(self->wtree,i);
